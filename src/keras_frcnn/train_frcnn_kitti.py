...
def on_batch_end(callback, names, logs, batch_no):
...
def train_kitti():
    ...
    cfg.simple_label_file = 'labels.txt'
...
# burning
    ...
    #plot_model(model_all, to_file='./model/model_all.png')

    log_path='./log'
    callback=TensorBoard(log_path)
    callback.set_model(model_rpn)
    c_callback=TensorBoard(log_path)
    c_callback.set_model(model_classifier)
    a_callback=TensorBoard(log_path)
    a_callback.set_model(model_all)
    ...
    for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                on_batch_end(a_callback, model_rpn.metrics_names, loss_rpn, epoch_num)
                ...
                on_batch_end(a_callback, model_classifier.metrics_names, loss_class, epoch_num)
